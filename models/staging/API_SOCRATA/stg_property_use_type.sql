...
with distinct_property_types as (
    ...
    select distinct
        trim(epapropertytype)::varchar(100) as property_use_type
    from {{ source('api_socrata', 'building_energy_benchmarking') }}
    ...
    select distinct
        trim(primarypropertytype)::varchar(100) as property_use_type
    from {{ source('api_socrata', 'building_energy_benchmarking') }}
    ...
    select distinct
        trim(value)::varchar(100) as property_use_type
    from {{ source('api_socrata', 'building_energy_benchmarking') }},
